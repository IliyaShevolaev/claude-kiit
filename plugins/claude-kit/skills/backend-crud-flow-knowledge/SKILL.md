...
description: "Knowledge base: the canonical backend CRUD vertical slice — Controller → custom Request → DTO (optional) → Service (+ base datatable service) with golden examples and project-specific pitfalls. Load this BEFORE implementing a new entity, endpoint, or CRUD feature so the typical wiring is done right."
---

# Backend CRUD flow (knowledge)

The canonical worked example of a backend CRUD vertical slice. Use it as the wiring template for a new entity / endpoint / typical CRUD task. For the underlying rules see the `backend-conventions-knowledge` canon — this skill shows those rules *applied*.
...
- **DTO** — only if the project uses a DTO layer (see CLAUDE.md); otherwise the validated request feeds the service directly.
- **Resource** — only when the task requires returning data. `store` / `update` are usually `void`.
- **Filter / Query classes** — only for list endpoints backed by the project's base datatable/list service (see CLAUDE.md).
- **Custom Rule object** — only for validation too complex for inline rules.
- **`HasOptionalFields` / `toFillable`** — only when the DTO has `Optional` fields used on `update`.
...
1. **Authorization** via `$this->authorize(...)` — either a permission name, or a policy check.
...
- If the project's base controller overrides `authorize()` so the **second argument is always an array** (see CLAUDE.md):
  ```php
...
> If the project has no DTO layer (see CLAUDE.md), pass the validated data instead, e.g. `$this->service->create($request->validated())`.
...
- `extends FormRequest`.
- **Never write `authorize(): bool`.** If omitted it defaults to `true`. Authorization lives in the controller via `$this->authorize()`; the method here is useless.
- `bail` is **mandatory** on every field — stop on the first failure.
- Rules are **always an array** `['required', 'string']`, never a pipe string `'required|string'`.
- Incoming ids → always validate existence excluding soft-deleted: `exists:...,id,deleted_at,NULL`.
- Enums → `Rule::enum(SomeEnum::class)`.
- Numbers → bound with `min` / `max` / `decimal`.
- Uniqueness (when business logic needs it) → scope out soft-deleted and ignore self on update: `Rule::unique('examples', 'email')->whereNull('deleted_at')->ignore($this->id)`.
- Complex validation → a custom Rule object.
- Need extra data not in the request to validate against → use `withValidator()`.
...
## 3. DTO — the boundary; the Request never travels further (if the project uses a DTO layer, see CLAUDE.md)
...
- Optional, must-not-be-null fields → type them `Optional` (not nullable). Use a trait such as `App\Traits\HasOptionalFields` → `toFillable()` so `update` does not overwrite model fields that were absent from the request.
- DTO → Model on `create` / `update` via `$dto->toArray()` (create) or `$dto->toFillable()` (update, when there are `Optional` fields). Fields land in the model's `$fillable`.
...
All business logic happens here: fetching the current user, branching, side effects, transactions, events.
...
The project's base datatable/list service (see CLAUDE.md) — the common approach when a service backs a table. It wraps the query in the project's datatable package (see CLAUDE.md) itself. You only provide:
- the query class implementing `DataTableQueryInterface`,
- the filter class implementing `DataTableFilterInterface`,
- the resource class:
  ```php
...
- [ ] Incoming ids use `exists:...,id,deleted_at,NULL`
