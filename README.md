...
4. `/layout-dev` — первый запуск собирает дизайн-основу проекта (глобальную тему, токены, шрифты, паттерны существующих компонентов) и переписывает скилл в `.claude/skills/layout-dev/SKILL.md` под конкретный дизайн.

Скилл `layout-dev` специализируется **сам при первом вызове**: он приходит как bootstrap-гайд, при первом использовании читает дизайн-основу проекта, переписывает себя под конкретный дизайн (глобально заданное не дублирует) и дальше работает как обычный design-system-гайд для вёрстки 1:1.
